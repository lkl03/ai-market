...
const uploadInfoToFirebase = async () => {
    ...
    await updateDoc(userRef, {
        ...
        userIsPaypalLogged: true
    }).then(() => {
        setMessage({
            status: 'success',
            text: 'You\'ve succesfully logged in with your PayPal account. Your product was succesfully sent to approval.'
        });

        const storagedData = JSON.parse(localStorage.getItem('savedData'));
    
        fetch("../api/approval", {
            "method": "POST",
            "headers": { "content-type": "application/json" },
            "body": JSON.stringify(storagedData)
        }).then((response) => {
            if (response.ok) {
                    ...
                    <motion.div className='max-w-[1920px] m-auto'>
                        ...
                        {snap.step1 && (
                            ...
                            <motion.div className={`absolute w-1/2 h-auto p-4 rounded-xl z-50 flex flex-col flex-wrap items-center justify-center gap-2 bg-[#fff]`}>
                                ...
                                <h2 className='2xl:text-[3rem] text-[1.5rem] font-black text-white'>{message.status === 'success' ? '¡Success!' : '¡Oops!'}</h2>
